...
```
The function 'songs' was then created in order to take each year's url data and transform it into table format.  This function takes in a url and a number n that can range from 1 to 100, and its default value is 10.  The input n represents the number of songs to be returned for each year.  Recall that the 'songs' function can only return up to 50 songs for the year 1958, but can generate up to 100 songs for every other year.  The tables this function returns include the song title, artist, and number ranking on the billboard chart.  The 'songs' function was called for each year in order to generate a table of the top 10 songs for each year from 1958 to 2021.  A coloumn designating year was then added to each year's table.  Lastly, all of the separate year tables were binded together by row to form a single data frame containing 620 songs. 
